...
In this two pointer solution, our final time complexity is _O(n^2)_ which we get from our sorting of `nums` + our `while` loop * checking for duplicates which equals $$O(n$$log$$n)$$ + $$O(n)$$ * $$O(n)$$ = $$O(n$$log$$n)$$ + $$O(n^2)$$ = $$O(n^2)$$. 
